...
if true then
	...
	assert.fails(function() linda:receive_batched("some key", -1, 1) end)
	assert.fails(function() linda:receive_batched("some key", 2, 1) end)
	assert.failsnot(function() linda:receive_batched(0, "some key", 1, 3) end)
	local fun = function() print "function test ok" end
	print(pcall(linda.set, linda, 'test', true, nil, fun))
	-- read back the contents
	local k,b,n,f = linda:receive_batched('test', 3)
	local _count, _val = linda:get("test")
	assert(_count == 0 and _val == nil)
	-- check they are ok
	print(k, b, n)
